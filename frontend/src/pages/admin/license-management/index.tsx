        ...
        items={[
          {
            ...
            render: (item) => <Link to={`/admin/license-management/${item.id}`}>{item.licenseCode}</Link>
          },
          {
            header: 'Mã hồ sơ',
            value: 'dossierCode',
            className: 'min-w-[150px] font-semibold text-blue-500 hover:underline',
            render: (item) => <Link to={`/admin/dossier-management/${item.dossierId}`}>{item.dossierCode}</Link>
          },
          {
            header: 'Tên doanh nghiệp (VI)',
            value: 'businessName',
            className: 'min-w-[200px] font-semibold text-blue-500 hover:underline',
            render: (item) => <Link to={`/admin/business-management/${item.businessId}`}>{item.businessName}</Link>
          },
