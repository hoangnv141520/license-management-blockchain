  ...
  const mutateSignUp = useSWRMutation(
    ...
    (_, { arg }: { arg: any }) => AuthService.signUp(arg.email, arg.password),
    {
      onSuccess: () => {
        ...
        showNotification('success', 'Đăng ký tài khoản thành công, chào mừng bạn đến với HyperFabric')
        navigate('/')
      },
      onError: (error: any) => {
      ...
      <Dialog open>
        <DialogContent className='rounded-lg sm:max-w-[450px] [&>button]:hidden'>
          ...
          <p className='text-center text-sm'>
            Bạn đã có tài khoản?{' '}
            <span className='cursor-pointer underline' onClick={() => navigate('/auth/sign-in')}>
              Đăng nhập
            </span>
          </p>
        </DialogContent>
